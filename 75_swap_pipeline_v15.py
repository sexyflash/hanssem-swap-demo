"""Swap pipeline v15 — v14 + 매핑 보강 + 컬러 hallucination QA + 컬러명 강제.

사용자 지적 (v14 후):
- panel_08 측면에 detail_03/05/p08_frame_04/05 가용했는데 2장만 → 보강
- 마스터 시트는 안 쓰기로 했으니 file도 archive ✓
- 사이즈/lineup/lifestyle도 가용 ref 더 활용 → 매핑 보강
- panel_12 컬러 hallucination ("풀리에이지/로즈브라운") + QA 99점 통과 → 진짜 문제
  · prompt에 컬러명 quote 강제 + swatch ref 추가
  · QA에 expected_milo_colors 정확 매칭 검증

v14 → v15 변경:
- panel_ref_mapping 보강 (panel당 3~5장)
- master sheet 완전 제거
- QA에 컬러 라벨 검증
- prompt에 milo 컬러명 quote 강조
"""
...
"""
    if panel_class == "color_option":
        colors_quoted = " / ".join(f'"{c}"' for c in COLOR_OPTIONS) if COLOR_OPTIONS else '"오트밀" / "라이트그레이" / "더스티로즈"'
        colors_only = ", ".join(f'"{c}"' for c in COLOR_OPTIONS) if COLOR_OPTIONS else '"오트밀", "라이트그레이", "더스티로즈"'
        return f"""\
EXPLICIT EDITS — color option panel:
[1] SOFAS: 4-seat leather (한샘 컬러) → 2-seat fabric MILO ({colors_quoted})
[2] LABELS (정확한 한글 텍스트만 사용 — 다른 컬러명 발명 X):
   허용된 milo 컬러명: {colors_only}
   금지: 한샘 컬러 ("그레이", "도브그레이", "스카이그레이") · hallucination ("풀리에이지", "로즈브라운", "베이지", "오트", "그레쥬" 등)
   각 swatch 아래 라벨은 정확히 위 3개 중 하나만 사용.
   Dimensions → "200 X 90 X 85"
[3] 다리: 4개 (모서리), 중앙 다리 X.
[4] 컬러 톤은 첨부된 IMAGE swatch (오트밀=#ECE2D2 톤, 라이트그레이=#C8C7C3 톤, 더스티로즈=#C9A5A1 톤) 그대로.
...
[1] SOFAS: 정확히 {N_MODELS}개 ({model_names}). 한샘 원본 N개와 무관하게 milo는 {N_MODELS}개만 표시.
    {models_str}
[2] 부족한 칸은 비우지 말고 {N_MODELS} 칸으로 레이아웃 재배치.
[3] LABELS:
   상단 헤더 (좌·우 통일): "밀로 패브릭소파 라인업"
   각 모델: 모델명 + 사이즈
[4] 컬러 라벨 (★ 사용자 강조):
   허용된 milo 컬러명: {colors_quoted}
   금지: 한샘 컬러 · hallucination ("풀리에이지", "로즈브라운", "베이지", "오트", "그레쥬" 등)
   컬러 swatch 아래 정확히 이 3개 한글명만 인쇄.
   첨부된 swatch IMAGE 의 색상을 정확히 보존.
[5] 다리: 4개 (모서리, 중앙 X).
"""
...
def qa(panel_orig, swap_result, panel_idx, panel_class):
    ...
    is_color_panel = panel_class in ("color_option", "module_lineup")
    extra = ""
    if is_chart:
    ...
    color_check = ""
    if is_color_panel:
        allowed = ", ".join(f'"{c}"' for c in COLOR_OPTIONS) if COLOR_OPTIONS else '"오트밀", "라이트그레이", "더스티로즈"'
        color_check = (
            f"\n**컬러 라벨 검증 (panel_12/color_option 핵심)**:\n"
            f"  허용된 milo 컬러명: [{allowed}]\n"
            f"  결과 B의 컬러 라벨이 정확히 위 list에 있는지 확인.\n"
            f"  hallucination 예시 (모두 fail): '풀리에이지', '로즈브라운', '베이지', '오트', '그레쥬'\n"
            f"  한샘 잔존 (모두 fail): '그레이', '도브그레이', '스카이그레이'\n"
            f"9) color_labels_match (1-5): 결과 B의 모든 컬러 라벨이 허용된 milo 컬러명과 일치?\n"
            f"   (한 개라도 hallucination 또는 한샘 잔존이면 score 1)\n"
        )
    ...
    qa_prompt = (
        ...
        "**critical 검증**:\n"
        "1) B의 image_format: 'single_product' / 'lifestyle' / 'chart_diagram' / 'multi_panel_grid'\n"
        "   multi_panel_grid = 즉시 retry\n"
        "2) B vs C layout 일치\n"
        ...
        + ('"color_labels_in_B":[모든 컬러명 list], "color_labels_match_milo":bool, "color_hallucinations":[잘못된 라벨 list], ' if is_color_panel else "")
        ...
        + (" - color_labels_match_milo == true (모든 컬러 라벨이 허용 list와 일치)\n" if is_color_panel else "")
    )
