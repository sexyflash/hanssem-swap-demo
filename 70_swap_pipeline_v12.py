"""Swap pipeline v12 — master sheet 복제 방지 + QA 형태 분류.

v11 (13/13 pass)의 함정: cover/panel_02/08가 master sheet 자체를 복제했는데
gpt-5.5 QA가 100점 부여 → 통계만 좋고 실제 결과는 cheat.

v12 fix:
1. swap prompt: master sheet는 정보 reference일 뿐, 시트 자체 복제 절대 X
2. swap prompt: 결과는 panel C 와 같은 single panel layout (multi-cell grid X)
3. QA에 image_format 분류 추가 (single_product / lifestyle / chart_diagram / multi_panel_grid)
4. multi_panel_grid 이면 즉시 retry 강제
5. QA에 layout_match_C (1-5) 추가: panel C 의 레이아웃 (정면샷 vs lifestyle vs chart)과 일치하는가
"""
...
REFERENCE PATTERN (이번 호출의 image_urls 순서):
  IMAGE 1 = panel C (한샘 원본 panel — **edit base, 결과는 이것의 layout/구도/카메라/크롭을 그대로 유지**)
  IMAGE 2,3,4 = MILO 메인 대표 사진 (정면/측면/3-4 각도) — 제품 외형 reference
  IMAGE 5 = MILO MASTER SHEET — **정보 reference 전용**.
            이 시트는 PRESERVE/NEVER 규칙과 디테일 설명을 운영팀이 정리한
            메타 자료입니다. **시트 자체를 결과로 복제/모방하지 마세요**.
            결과 이미지는 IMAGE 1 (panel C)와 같은 single panel 형태여야 하며,
            multi-cell grid 또는 sheet 형태 절대 X.

{target_summary()}

=== [출력 형식 강제] ===
- 결과는 **IMAGE 1 (panel C)와 동일한 single-panel layout**.
- panel C가 정면 hero shot이면 → 정면 hero shot 1장
- panel C가 lifestyle scene이면 → 같은 룸 lifestyle scene 1장
- panel C가 size chart면 → 동일한 chart 구조 1장
- **결과가 multiple cells 또는 grid 형태로 나오면 fail**.
- IMAGE 5 (master sheet)의 cell-grid 구성을 결과에 옮기지 마세요.
...
- 결과는 panel C와 같은 single panel, 한국 가구 상세페이지 visual style.
- 모든 한글 텍스트 깨짐 X.
- 다리는 정확히 4개 (모서리 4개, 중앙 5번째 다리 절대 X) — hallucination 금지.
- 한샘 원본 가죽-특정 잔존 요소 (배지/플레이트/리클라이너 하드웨어/V자 다리) 제거.
- IMAGE 5 master sheet의 PRESERVE 규칙 반영, NEVER 항목 violate X.
- **IMAGE 5 시트 layout 자체는 복제하지 마라** — 정보만 참고하고 결과는 panel C 형식.
- 좌석수 정확히 2.
- 모듈 라인업이면 milo 모델 옵션 갯수만큼 정확히 ({N_MODELS}개).
- color/lineup panel의 컬러 라벨: 한샘 컬러명 ("그레이/도브그레이/스카이그레이") 잔존 X,
  milo 컬러명 ("오트밀/라이트그레이/더스티로즈")로 정확히 교체.
"""
...
"""
    ...
    qa_prompt = (
        ...
        "**critical 검증 항목**:\n"
        "1) B의 image_format 분류 (가장 중요):\n"
        "   - 'single_product': 단일 product shot 1장 (panel C 가 product shot일 때 정답)\n"
        "   - 'lifestyle': 룸/공간 안의 가구 1장 (panel C 가 lifestyle 일 때 정답)\n"
        "   - 'chart_diagram': size/structure chart 같은 정보 도표 1장 (chart panel 정답)\n"
        "   - 'multi_panel_grid': 여러 셀로 분할된 grid/sheet (PRESERVE/NEVER 박스, 컬러 chip,\n"
        "     디테일 close-up이 한 이미지에 모자이크 형태로 들어가 있음) — 이건 master sheet를\n"
        "     복제한 fail 케이스. 즉시 retry.\n"
        "2) B와 C의 layout 일치 여부 (B가 C 와 같은 단일 panel 형태인가)\n"
        "3) 다리는 정확히 4개 (5개면 hallucination)\n"
        "4) 한샘 잔존 요소 (가죽 sheen, V자 다리, 배지, 한샘 컬러명 \"그레이/도브그레이\") 제거 여부\n\n"
        "5점 만점:\n"
        "1) arm_thinness\n"
        "2) cushion_loose\n"
        "3) fabric_matte\n"
        "4) leg_count_4 (다리 4개면 5, 5개면 1)\n"
        "5) layout_match_C (B가 C와 같은 layout 형식이면 5, multi-grid면 1)\n"
        f"{extra}"
        "JSON: {\"image_format\":\"single_product|lifestyle|chart_diagram|multi_panel_grid\", "
        "\"scores\":{...}, \"product_score\":0-100, \"composition_score\":0-100, "
        "\"seat_count_in_B\":int, \"leg_count_in_B\":int, "
        "\"underswap\":bool, \"text_in_image_garbled\":bool, "
        "\"hanssem_text_residue\":bool, "
        "\"specific_failures\":[한국어], "
        "\"reference_influences\":[{\"element_ko\":,\"source\":\"C\",\"correction_ko\":}], "
        "\"notes\":\"한국어\", \"verdict\":\"pass\"|\"retry\"}\n\n"
        "**pass 기준 (v12 강화)**:\n"
        " - image_format != 'multi_panel_grid' (필수)\n"
        " - layout_match_C >= 5\n"
        " - ALL scores >= 5\n"
        " - product_score >= 99\n"
        " - leg_count_in_B == 4\n"
        " - NOT underswap, NOT text_in_image_garbled, NOT hanssem_text_residue"
    )
    ...
    # v12 강제 retry — 형태 분류
    fmt = result.get("image_format", "")
    if fmt == "multi_panel_grid":
        result["verdict"] = "retry"
        result.setdefault("specific_failures", []).insert(0,
            "결과가 master sheet 같은 multi-panel grid 형태 — single panel로 재생성 필요")
        result["product_score"] = min(result.get("product_score", 0), 50)
    ...
    if result.get("hanssem_text_residue"):
        result["verdict"] = "retry"
        result.setdefault("specific_failures", []).insert(0,
            "한샘 텍스트 (그레이/도브그레이/스카이그레이 또는 4인용 dim) 잔존 — milo 컬러명/dim 으로 교체")
...
async def run_target(label, base_img, layout_map, urls, image_size, sem):
    ...
    for attempt_n in range(1, MAX_ATTEMPTS + 1):
        ...
        async with sem:
            ...
            img = OUT_DIR / f"{label}__a{attempt_n}_v12.png"
            try:
...
async def main():
    ...
    (OUT_ROOT / "latest_v12.txt").write_text(RUN_DIR.name)
