...
# product_input_v2 — 정밀 vision 재추출 (쿠션 연결/시접/다리 위치 등 상세)
_pi_v2 = milo_dir / "product_input_v2.json"
_pi_v1 = milo_dir / "product_input.json"
product_input = json.loads(_pi_v2.read_text()) if _pi_v2.exists() else (json.loads(_pi_v1.read_text()) if _pi_v1.exists() else {})
...
def build_zone_inputs():
    """이미지 zone — 원본 업로드 + 전처리 산출물(strict95 패턴) 모두 표시.
    썸네일 클릭 시 모달로 확대."""
    use_prod = "../user_products/milo_777039/use_product"
    milo = "../user_products/milo_777039"
    prep = "../user_products/milo_777039/preprocessed"

    zones_data = [
        # 원본 업로드 zone들
        ("📷 제품 이미지 (원본)", [
            (f"{use_prod}/p08_frame_01.jpg", "정면 (오트밀)"),
            (f"{use_prod}/p08_frame_02.jpg", "측면 A"),
            (f"{use_prod}/p08_frame_03.jpg", "측면 B"),
            (f"{milo}/panel_01.jpg", "Hero"),
            (f"{milo}/_cover_1000.jpg", "Cover 1:1"),
        ]),
        ("🧵 소재 (원본)", [
            (f"{use_prod}/material.png", "패브릭 close-up"),
        ]),
        ("🎨 컬러 (원본 swatch)", [
            (f"{use_prod}/color_01.png", "오트밀"),
            (f"{use_prod}/color_02.png", "라이트그레이"),
            (f"{use_prod}/color_03.png", "더스티로즈"),
        ]),
        ("📦 라인업 (컬러 variants)", [
            (f"{use_prod}/p08_frame_01.jpg", "오트밀"),
            (f"{use_prod}/p08_frame_01_color_2.webp", "라이트그레이"),
            (f"{use_prod}/p08_frame_01_color_3.webp", "더스티로즈"),
        ]),
        # 전처리 zone (strict95 패턴 포함)
        ("⭐ 전처리 — strict95 패턴 (swap 시 항상 첨부)", [
            (f"{prep}/milo_smart_sheet.jpg", "Smart Sheet (PRESERVE/NEVER)"),
            (f"{prep}/product_grid.jpg", "Product Grid 6-cell"),
            (f"{prep}/material_close.jpg", "Material 확대"),
            (f"{prep}/color_chart.jpg", "Color Chart 3종"),
        ]),
    ]
    out = ""
...
def build_text_input_fields():
    """텍스트 입력 영역 — 풍부한 필드 (정밀 vision 추출 기반)."""
    p = product_input or {}
