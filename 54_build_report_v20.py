...
product_input = json.loads((milo_dir / "product_input.json").read_text()) if (milo_dir / "product_input.json").exists() else {}
...
def get_swap_results(t):
    ...
    for marker in ["latest_v9.txt", "latest_v8.txt", "latest_v7.txt", "latest_v6.txt", "latest_v5.txt", "latest_v4.txt", "latest.txt"]:
        latest = sr / marker
        if latest.exists():
...
def zone_refs_for_class(panel_class):
    ...
    elif panel_class in ("color_option", "module_lineup"):
        refs += [
...
''', "html.parser")
            ...
            if cell and cell.get("final") and cell["final"].get("image"):
                ...
                payload = {
                    ...
                    "original_panel": f"../templates/01_sofa_992474/panel_{panel_idx:02d}.jpg",
                }
...
def build_zone_inputs():
    """이미지 zone 분리 — 이미지 업로드 자료만."""
    use_prod = "../user_products/milo_777039/use_product"
    zones_data = [
        ("제품 이미지 zone", [f"p08_frame_0{i}.jpg" for i in [1, 2, 3]]),
        ("소재 zone", ["material.png"]),
        ("컬러 zone", [f"color_0{i}.png" for i in [1, 2, 3]]),
    ]
    out = ""
    for title, files in zones_data:
        thumbs = "".join(f'<div class="zone-thumb"><img src="{use_prod}/{f}" loading="lazy"/></div>' for f in files)
...
def build_text_input_fields():
    """텍스트 입력 영역 — 필드별로 분리 (운영 시 담당자가 채울 자리)."""
    p = product_input or {}
    ...
    features_str = "\n".join(f"· {f}" for f in (feat.get("key_features_korean", []) or [])[:8])

    fields = [
        ("제품명", "product_name", p.get("product_name", "")),
        ("제품 소개 (헤드)", "intro_headline", intro.get("headline", "")),
        ("제품 소개 (본문)", "intro_body", intro.get("body", "")),
        ("제품 제원 (사이즈)", "spec_size", f"{sp.get('width_cm','?')}×{sp.get('depth_cm','?')}×{sp.get('height_cm','?')}cm / 좌방석 깊이 {sp.get('seat_depth_cm','?')}, 좌방석 높이 {sp.get('seat_height_cm','?')}, 팔걸이 높이 {sp.get('arm_height_cm','?')}, 다리 {sp.get('leg_height_cm','?')}"),
        ("좌방석 firmness (5단계)", "seat_firmness", feat.get("seat_cushion_firmness", "")),
        ("등쿠션 firmness (5단계)", "back_firmness", feat.get("back_cushion_firmness", "")),
        ("좌방석 내장재", "seat_fill", feat.get("seat_cushion_fill", "")),
        ("등쿠션 내장재", "back_fill", feat.get("back_cushion_fill", "")),
        ("마감재", "upholstery", feat.get("upholstery", "")),
        ("프레임", "frame", feat.get("frame", "")),
        ("다리", "legs", feat.get("legs", "")),
        ("인증", "certs", certs_str),
        ("컬러 옵션", "colors", " / ".join(color_names)),
        ("모델 옵션 (선택: " + selected_model.get("name", "?") + ")", "models", model_names),
        ("제품 특징", "features", features_str),
    ]

    out = '<div class="input-fields-grid">'
    for label, key, val in fields:
        is_textarea = len(str(val)) > 40 or "\n" in str(val)
        if is_textarea:
            out += f'<div class="input-field"><label>{label}</label><textarea name="{key}" rows="3">{val}</textarea></div>'
        else:
            out += f'<div class="input-field"><label>{label}</label><input type="text" name="{key}" value="{val}"/></div>'
    out += '</div>'
    return out
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* zone row — 3개 zone 한 줄 */
  .zone-row {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 8px; }}

  /* 텍스트 입력 필드 grid */
  .input-fields-grid {{
    display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 10px 14px;
    margin-top: 8px;
  }}
