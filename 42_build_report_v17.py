...
function showPayload(btn) {{
  ...
  (data.attempts || []).forEach((a, idx) => {{
    ...
    const failsHtml = (a.specific_failures||[]).map(f => `<li>${{f}}</li>`).join('') || '<li style="background:#f0fdf4;border-left-color:var(--good);color:var(--good)">(none)</li>';
    const riHtml = (a.reference_influences||[]).map(r => `<li><b>${{r.element||'?'}}</b> from <b>${{r.source||'?'}}</b> → ${{r.correction||'?'}}</li>`).join('') || '<li style="background:#f0fdf4;border-left-color:var(--good);color:var(--good)">(none)</li>';
...
<div class="attempt-detail ${{active}}" data-att="${{idx}}">
  ...
  <div class="modal-section">
    <div class="modal-section-h">📋 Notes (QA 코멘트)</div>
    <div style="font-size:11px;color:var(--fg-soft);line-height:1.6">${{a.notes||'(none)'}}</div>
  </div>
</div>
`;
  }});
