...
PRODUCT_INPUT_FILE = USER_DIR / "product_input_v3.json"
...
def load_inputs():
    ...
    return layouts, classification, product_input
...
async def main(rerun_labels: set[str] | None = None):
    ...
    layouts, classification, product_input = load_inputs()
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="v16+ swap pipeline — full run or per-panel rerun"
    )
    p.add_argument(
        "--rerun",
        help="쉼표 구분 panel label list (e.g. 'cover,panel_03'). 지정 시 latest run dir에 partial rerun."
    )
    return p.parse_args()
